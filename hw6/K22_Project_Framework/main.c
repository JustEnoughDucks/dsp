...
void PIT0_IRQHandler(void){	//This function is called when the timer interrupt expires
	...
	{
		case 0: {
			Yn[Korder] = (float) valADC;
			break;
		}
		case 1: {
			for(int i = 0; i < Korder; i++)
			{
					Yn[i+1] = B1[i][0]*Yn[i] + s[i][0];
					s[i][0] = B1[i][1]*Yn[i] - A1[i][0]*Yn[i+1] + s[i][1];
					s[i][1] = B1[i][0]*Yn[i] - A1[i][1]*Yn[i+1];
			}
			break;
		};
		case 2: {
			for(int i = 0; i < Korder; i++)
			{
					Yn[i+1] = B2[i][0]*Yn[i] + s[i][0];
					s[i][0] = B2[i][1]*Yn[i] - A2[i][0]*Yn[i+1] + s[i][1];
					s[i][1] = B2[i][0]*Yn[i] - A2[i][1]*Yn[i+1];
			}
			break;
		};
		case 3: {
			for(int i = 0; i < Korder; i++)
			{
					Yn[i+1] = B3[i][0]*Yn[i] + s[i][0];
					s[i][0] = B3[i][1]*Yn[i] - A3[i][0]*Yn[i+1] + s[i][1];
					s[i][1] = B3[i][0]*Yn[i] - A3[i][1]*Yn[i+1];
			}
			break;
		}
		case 4: {
			for(int i = 0; i < Korder; i++)
			{
					Yn[i+1] = B4[i][0]*Yn[i] + s[i][0];
					s[i][0] = B4[i][1]*Yn[i] - A4[i][0]*Yn[i+1] + s[i][1];
					s[i][1] = B4[i][0]*Yn[i] - A4[i][1]*Yn[i+1];
			}
			break;
		}
	}
