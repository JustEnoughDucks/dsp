...
for Freq = Ftest
    ...
    cents(count,count2) = 1200*log2(f0(count,count2)/Freq);

    count = count + 1;

end
    centsAvg(count2) = mean(abs(cents(:,count2)));
    count = 1;
    count2 = count2 + 1;
end

[optiErr, optiFreqInd] = min(centsAvg);
optiFreq = Frange(optiFreqInd)
semilogx(Ftest, cents(:,optiFreqInd));
title('Cent Error of Optimal Low Sample Frequency');
...
semilogy(Ftest);
hold on
semilogy(f0(:,optiFreqInd));
hold off

title('Actual Frequency vs. Pure Sin Wave Estimate of Optimal Low Sample Freq');
xlabel('52-length frequency index');
ylabel('Frequency (Hz)');
legend('Input Sin Wave', 'Frequency Estimate')